...
if years_selected and len(years_selected) <= 1:
    xAxisName = "Mes"
    cy_surveysdf_date = surveyusagedf.groupby('Mes')['Nps'].sum().reset_index(name ='Nps')
else:
    xAxisName = "Año"
    cy_surveysdf_date = surveyusagedf.groupby('Año')['Nps'].sum().reset_index(name ='Nps')
...
cy_surveysdf_espe = surveyusagedf.groupby('Speciality')['Nps'].sum().reset_index(name ='Nps') # .sort_values(by='surveys',ascending=False)
